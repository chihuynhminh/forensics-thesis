...
# Check pte function
def check_pt_func_nopae(pde, pte):
...
# Check pde function
def check_pd_func_nopae(pde):
    if (bitAt(pde, 0) == 0): return 0
    if (bitAt(pde, 7) == 1):
        if(bitFromTo(pde, 17, 21) != 0): return -1
    return 1

# Scan memdump with 2 process name
def scan_pd_nopae(pname1, pname2):
    ...
    for i0 in range(0, mem_size, 4096):
        ...
        w = scanPageDirectory_nopae(i0, check_pd_func_nopae, check_pt_func_nopae, i0, page_indexs_1, page_indexs_2, mem_size, page_indexs_1_4MB, page_indexs_2_4MB)
        if(w > 0):
            cr3_list.append(i0)
            w_cr3.append(w)
